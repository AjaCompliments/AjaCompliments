...
module.exports = {
  ...
  modifyPass:(req,res)=>{
    try{
       ...
       var sql=`update users set userpass="${hashed}" where iduser=${req.body.id}`
  ...
  updateThroughManager:(req,res)=>{
   ...
   //     console.log(!req.headers.authorization||2, "req.heas.auth",
    ...
    //         }
        ...
        // Iterate through req.body fields
        for (const key in req.body) {
            if (req.body.hasOwnProperty(key) && key !== 'iduser') {
                // Exclude 'iduser' from fields
                ...
                if (value !== '') {
                    ...
                    // Escape and add to the list of fields to update
                    fieldsToUpdate.push(`${key}=${db.escape(value)}`);
                }
            }
        }
        ...
        const updateQuery = `UPDATE users SET ${setClause} WHERE iduser=${db.escape(req.body.iduser)}`;
